...
class GraspQualityConfig(BaseConfig):
    """Serializable contact and clearance thresholds shared by all grasp variants."""

    model_config = {"frozen": True, "allow_inf_nan": False}

    min_object_points: int = Field(default=150, gt=0)
    voxel_size_m: float = Field(default=0.003, gt=0)
    min_occupied_voxels: int = Field(default=24, gt=0)
    min_capture_points: int = Field(default=50, gt=0)
    min_closing_span_m: float = Field(default=0.006, gt=0)
    max_tilt_deg: float = Field(default=60.0, gt=0, lt=90)
    tilt_score_floor: float = Field(default=0.5, gt=0, le=1)
    min_raw_score: float = Field(default=0.7, gt=0, le=1)
    min_capture_fraction: float = Field(default=0.25, gt=0, le=1)
    min_contact_points_per_side: int = Field(default=12, gt=0)
    min_contact_normal_alignment: float = Field(default=0.25, gt=0, le=1)
    jaw_margin_m: float = Field(default=0.003, gt=0)
    clearance_m: float = Field(default=0.003, gt=0)
    diversity_distance_m: float = Field(default=0.01, gt=0)
    diversity_angle_deg: float = Field(default=15.0, gt=0)
    max_candidates: int = Field(default=100, gt=0)


def pose_matrix(candidate: GraspCandidate) -> np.ndarray:
    matrix = np.eye(4)
...
def similar_grasp(a: np.ndarray, b: np.ndarray, distance: float, angle_deg: float) -> bool:
    ...
    angles = [
        np.degrees(np.arccos(np.clip((np.trace(relative @ symmetry) - 1) / 2, -1, 1)))
...
class GraspQualityFilter:
    def __init__(
        ...
        grasp_frame_to_tcp: list[list[float]],
    ...
    def filter(
        ...
        *,
        ...
        failed_grasps: list[np.ndarray] | None = None,
        ...
        if (
            candidates.header.frame_id != object_cloud.frame_id
            or scene_cloud.frame_id != object_cloud.frame_id
        ):
        ...
        normals = vectors[:, :, 0]
        normals *= np.where(np.sum(normals * (points - points.mean(axis=0)), axis=1) < 0, -1, 1)[
            :, None
        ]
        ...
        for candidate in candidates.candidates:
            ...
            if tilt > cfg.max_tilt_deg:
                ...
                continue
            if any(
                similar_grasp(tcp, failed, 2 * cfg.diversity_distance_m, cfg.diversity_angle_deg)
            ...
            left = (
                capture
                & (local[:, 0] <= lo + width * 0.25)
                & (local_normals[:, 0] <= -cfg.min_contact_normal_alignment)
            )
            right = (
                capture
                & (local[:, 0] >= hi - width * 0.25)
                & (local_normals[:, 0] >= cfg.min_contact_normal_alignment)
            )
            if min(int(left.sum()), int(right.sum())) < cfg.min_contact_points_per_side:
                reject("insufficient_opposing_contacts")
                continue
            reason = self.geometry.obstruction(
                tcp,
                scene,
                tree,
                points,
            ...
            tilt_quality = (
                cfg.tilt_score_floor + (1 - cfg.tilt_score_floor) * np.cos(np.radians(tilt)) ** 2
            )
            quality = (
                float(candidate.score)
                * tilt_quality
                * (0.5 + 0.5 * np.mean([center_quality, balance, n / len(points)]))
            )
            metrics = {
                ...
                "tilt_deg": tilt,
                "capture_points": n,
                "capture_fraction": n / len(points),
                "closing_span_m": float(width),
                "left_contact_points": int(left.sum()),
                "right_contact_points": int(right.sum()),
            }
        ...
        for item in survivors:
            if any(
                similar_grasp(item[2], other[2], cfg.diversity_distance_m, cfg.diversity_angle_deg)
        ...
        return GraspCandidateArray(
            candidates.header, [GraspCandidate(item[1].pose, score=item[0]) for item in selected]
        ), report
